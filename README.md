...
Sensor data is superimposed onto a two-octave pitch lattice based on the **tonic**, **scale**, and **starting octave** of your choosing. Here's how it works:

- **Tonic**: The root note of your selected scale (e.g., C, D#, G).
- **Scale**: The interval pattern used to convert data values into musical steps (e.g., Major, Minor, Pentatonic).
- **Tessitura**: The vertical pitch range (in octaves) within which notes are generated.

Data values are normalized and converted to MIDI note values using the selected scale and tonic, wrapping the data onto a musical grid for coherent musicality.
